...
test('широкий forward-поиск: перескакивает на текст на много слов вперёд', () => {
  ...
  try {
    ...
    // Слова 58-62 («каждую осень студенты готовятся к») — на 62 слова
    // дальше курсора, за пределами старого LOOKAHEAD=30. Новый
    // LOOKAHEAD=100 должен поймать (проверено отдельно: единственная
    // позиция в окне [0,100) с score>=0.6 — это позиция 62).
    speak(rec, 'каждую осень студенты готовятся к');

    assert.equal(v.cursor, 62, 'курсор перескочил далеко вперёд');
  } finally {
    v.stop();
  }
});
...
test('широкий backward-поиск: перескакивает на текст на много слов назад', () => {
  ...
  try {
    ...
    // курсора, за пределами старого LOOKBACK=50. Новый LOOKBACK=150
    // должен поймать (единственная позиция в окне [0,120) с score>=0.6).
